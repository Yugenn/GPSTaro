<x-app-layout>
    <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-indigo-900 shadow-md rounded-md">
        ...
        <form action="{{ route('ad_offers.update', $adOffer) }}" method="POST" class="rounded pt-3 pb-8 mb-4">
            @csrf
            @method('PUT')
            <div class="mb-4">
                ...
                <input type="text" name="title"
                    ...
                    required placeholder="タイトル" value="{{ old('title', $adOffer->title) }}">
            </div>
            <div class="mb-4">
                ...
                <select name="area_id"
                    ...
                    @foreach ($areas as $area)
                        <option value="{{ $area->id }}" @if ($area->id == old('area_id', $adOffer->area_id)) selected @endif>{{ $area->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <input type="date" name="remaining_amount"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="募集期限" value="{{ old('remaining_amount', $adOffer->remaining_amount) }}">
            </div>
            <div class="mb-4">
                ...
                <textarea name="description" rows="10"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="詳細">{{ old('description', $adOffer->description) }}</textarea>
            </div>
            <div class="mb-4">
                <label class="block text-white mb-2" for="description">
                    公開状況
                ...
                <select name="status"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3">
                    @foreach (AdOfferConst::STATUS_LIST as $name => $value)
                        <option value="{{ $value }}" @if ($value == old('status', $adOffer->status)) selected @endif>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
